...
from lerobot.inference.act_inference import ACTInference
...
def parse_args():
    ...
    model_group.add_argument(
        "--hf_model", type=str, help='Hugging Face model ID (e.g., "username/model-name")'
    )
    model_group.add_argument(
        "--local_checkpoint", type=str, help="Path to local checkpoint directory containing model files"
    )
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="minjunkevink/trossen_objects_pick_place",
        help="Hugging Face dataset ID to use for inference",
    )
    parser.add_argument("--split", type=str, default="train", help="Dataset split to use")
    parser.add_argument("--sample_idx", type=int, default=0, help="Index of the sample to use for inference")

    # Config arguments
    parser.add_argument(
        "--config_path", type=str, help="Path to config file (only needed for local checkpoints)"
    )

    args = parser.parse_args()

    # Always use the local model
    args.local_checkpoint = "/data/shared/train_act_trossen_pathmask/checkpoints/last/pretrained_model"
...
def load_model(args):
    ...
    model_dir = os.path.expanduser("/data/shared/train_act_trossen_pathmask/checkpoints/last/")
    ...
    # return TrossenActionWrapper(env, config_path, model_dir)
