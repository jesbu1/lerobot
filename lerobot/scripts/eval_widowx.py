...
def run_inference_loop(
    ...
    try:
        ...
        while True:  # Loop until stop, reset, or error
            ...
            action_chunk = action_chunk[:MAX_CHUNK_SIZE]
...
def save_trajectory(
    saver: RawSaver, episode_idx: int, raw_obs_list: List[Dict], action_list: List[np.ndarray]
):
    ...
    try:
        ...
        executed_actions = np.array(
            [chunk[0] for chunk in action_list if chunk.ndim >= 2 and chunk.shape[0] > 0]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run inference on USC WidowX robot using OpenPI policy server."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--robot-ip", type=str, default="localhost", help="IP address of the WidowX robot controller."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="./trajectory_data/usc_widowx",
